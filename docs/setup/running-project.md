...
4. If prompted to select a workspace, choose `/workspaces/devtest/` (the same path used by the DevContainer in VS Code and PyCharm).
5. Inside the codespace, copy `.env.example` to `.devcontainer/.env` if it is missing.
6. Select the interpreter `./.venv/bin/python` if prompted.
7. Use the VS Code interface in the browser to run tasks and manage the server.
8. Seed data can be loaded in the terminal with `python manage.py seed_data`.
