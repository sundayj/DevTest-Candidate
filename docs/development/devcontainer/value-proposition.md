...
- **postCreateCommand**: `"uv sync && python manage.py migrate"` - Installs dependencies and applies migrations automatically
- **postAttachCommand**: `"uv sync"` - Ensures environment is always up-to-date
- **workspaceFolder**: `"/app"` - Sets correct working directory
